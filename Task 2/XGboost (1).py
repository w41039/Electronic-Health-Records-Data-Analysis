...
y_valid = pd.read_csv("Y_valid_T1.csv")
...
def objective(trial,data=X_train,target=y_train):
    ...
    model = xgb.XGBRegressor(**param, missing = 0, scale_pos_weight = 143)
    model.fit(X_train, y_train, eval_set=[(X_valid, y_valid)],early_stopping_rounds=100,verbose=False)
    y_valid_scores = model.predict(X_valid)[:, 1]
    rmse_valid = mean_squared_error(y_valid, y_valid_scores, squared = False)
    average_rmse.append(rmse_valid)
...
print('average_rmse: ', sum(average_rmse)/50)
